...
class TestGraphDefinition:
    ...
    def test_graph_has_transitions(self):
        graph = build_content_pipeline_graph()
        # The pipeline defines exactly 16 transitions (verified against workflow.py)
        assert len(graph.transitions) == 16
...
class TestScreenContentAction:
    ...
    async def test_screen_bypasses_llm_when_reasoning_enabled(self, agent_settings, channel, sample_items):
        """When reasoning_enabled=True, screen_content passes all items through without LLM call."""
        from app.agent.channel.workflow import screen_content
        ...
        result = await screen_content(state)
        assert result["relevant_items"] == sample_items
        assert result["error"] is None

    async def test_screen_handles_screening_error(self, channel, sample_items):
        from app.agent.channel.config import ChannelAgentSettings
        from app.agent.channel.exceptions import ScreeningError
        from app.agent.channel.workflow import screen_content

        settings_no_reasoning = ChannelAgentSettings(
            enabled=True,
            discovery_enabled=False,
            source_discovery_enabled=False,
    ...
    async def test_screen_handles_generic_error(self, channel, sample_items):
        from app.agent.channel.config import ChannelAgentSettings
        from app.agent.channel.workflow import screen_content
...
class TestFullPipeline:
    ...
    async def test_pipeline_halts_at_review(self, channel, agent_settings, mock_bot, mock_session_maker, sample_items):
        ...
        with (
            ...
            patch(
                "app.agent.channel.reasoning.evaluate_batch",
                return_value=[(sample_items[0], MagicMock(decision="post", reasoning="good"))],
            ),
            patch("app.agent.channel.generator.generate_post", return_value=mock_post),
            patch("app.agent.channel.feedback.get_feedback_summary", return_value=None),
