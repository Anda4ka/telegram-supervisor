...
def register_channel_tools(agent: Agent[AssistantDeps, str]) -> None:
    ...
    async def add_source(
        ...
        """Add a content source for a channel.

        For Twitter sources use source_type='twitter' and url like 'twitter:@username'.
        For RSS sources use source_type='rss' (default) and a regular RSS URL.
        For Telegram channels use source_type='telegram' and url like 'tg:channel:ID'.
        """
        ...
        try:
            ...
            return f"Added {source_type} source: {url} for {channel_id}"
        except Exception:
    ...
    async def remove_source(ctx: RunContext[AssistantDeps], url: str, channel_id: int) -> str:
        """Remove a source by URL for a specific channel. Works for any source type (rss, twitter, telegram). channel_id is required."""
        from sqlalchemy import select
