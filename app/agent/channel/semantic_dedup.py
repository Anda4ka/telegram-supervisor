...
async def find_nearest_posts_batch(
    texts: list[str],
    *,
    channel_id: int,
    api_key: str,
    ...
    """Batch version of find_nearest_posts — single embedding API call for all texts.

    Returns a list of results per input text. Each result is a list of
    (title, similarity_score) tuples for the nearest post (max 1 per text).
    """
    if not texts:
        return []

    truncated = [t[:200] for t in texts]
    try:
        embeddings = await get_embeddings(truncated, api_key=api_key, model=model)
    except Exception:
        logger.warning("batch_nearest_embedding_error", exc_info=True)
        return [[] for _ in texts]

    max_distance = 1.0  # Return all, caller filters by threshold

    # Build VALUES list for batched query
    values_parts: list[str] = []
    params: dict[str, object] = {
        "channel_id": channel_id,
        "days": lookback_days,
    ...
    query = text(f"""
        SELECT c.idx, cp.title, 1.0 - (cp.embedding <=> c.vec) as similarity
        FROM (VALUES {values_sql}) AS c(idx, vec)
        CROSS JOIN LATERAL (
            SELECT cp2.title, cp2.embedding
            FROM channel_posts cp2
            WHERE cp2.channel_id = :channel_id
              AND cp2.embedding IS NOT NULL
              AND cp2.created_at > NOW() - make_interval(days => :days)
            ORDER BY cp2.embedding <=> c.vec
            LIMIT 1
        ) cp
        WHERE (cp.embedding <=> c.vec) < :max_distance
    """)  # noqa: S608

    results_map: dict[int, list[tuple[str, float]]] = {i: [] for i in range(len(texts))}
    async with session_maker() as session:
        result = await session.execute(query, params)
        for row in result.fetchall():
            idx, title, sim = int(row[0]), row[1], float(row[2])
            results_map[idx].append((title, sim))
