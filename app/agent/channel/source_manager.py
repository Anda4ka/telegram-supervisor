...
async def add_source(
    ...
    async with session_maker() as session:
        ...
        session.add(source)
        await session.commit()
        logger.info("source_added", url=url, source_type=source_type, added_by=added_by)
        return True
...
async def seed_sources_from_env(
    ...
    for url in rss_urls:
        if await add_source(session_maker, channel_id, url, added_by="config"):
            added += 1
    for account in twitter_accounts or []:
        username = account.lstrip("@")
        url = f"twitter:@{username}"
        if await add_source(
            session_maker,
            channel_id,
            url,
            ...
            added += 1
    for sub in reddit_subreddits or []:
