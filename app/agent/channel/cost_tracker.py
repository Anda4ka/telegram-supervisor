"""LLM cost tracking for channel agent — in-memory accumulation + DB persistence + structured logging."""
...
def set_usage_session_maker(sm: async_sessionmaker[AsyncSession]) -> None:
    """Set the session maker for DB persistence (called once at startup)."""
    global _session_maker  # noqa: PLW0603
    _session_maker = sm


async def _persist_to_db(usage: LLMUsage) -> None:
    """Persist a usage record to the database (best-effort)."""
    ...
    try:
        ...
        logger.warning("llm_usage_persist_failed", exc_info=True)


async def log_usage(usage: LLMUsage) -> None:
    """Log a single LLM usage record, accumulate in memory, and persist to DB.
...
async def get_db_summary(days: int = 30) -> dict[str, Any]:
    """Return usage summary from the database for the last N days."""
    if _session_maker is None:
    ...
    cutoff = utc_now() - __import__("datetime").timedelta(days=days)
    async with _session_maker() as session:
        result = await session.execute(
            select(
                ...
                func.sum(LLMUsageLog.cache_savings_usd).label("savings"),
    ...
    for row in rows:
        by_operation[row.operation] = {
            ...
            "cost_usd": round(float(row.cost or 0), 8),
            "calls": int(row.calls or 0),
            "cache_savings_usd": round(float(row.savings or 0), 8),
        }
