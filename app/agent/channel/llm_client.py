...
def _is_transient_pydanticai_error(exc: BaseException) -> bool:
    """Detect transient errors from PydanticAI agent.run() calls.

    PydanticAI wraps HTTP errors in ModelHTTPError with status_code.
    Also retries on httpx timeouts that may bubble up.
    """
    # Direct httpx errors (may bubble through PydanticAI)
    if isinstance(exc, httpx.TimeoutException):
        return True
    if isinstance(exc, httpx.HTTPStatusError):
...
_PYDANTICAI_RETRY = retry(
    retry=retry_if_exception(_is_transient_pydanticai_error),
    stop=stop_after_attempt(3),
...
async def run_agent_with_retry(agent: Any, prompt: str, **kwargs: Any) -> Any:
    """Run a PydanticAI agent with transient error retry.

    Wraps agent.run() with exponential backoff for transient HTTP errors
    (429, 5xx) from OpenRouter.
    """
    ...
    async def _call() -> dict[str, Any]:
        ...
        resp = await client.post(
            ...
            headers={
                ...
                "HTTP-Referer": "https://github.com/Anda4ka/telegram-supervisor",
            },
            json=payload,
