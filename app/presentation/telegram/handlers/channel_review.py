...
async def on_review_action(callback: CallbackQuery, callback_data: ReviewAction) -> None:
    ...
    if action == "approve":
        ...
        # Show time preset picker with best-time recommendation
        ...
        try:
            ...
            channel = await _get_channel_for_post(post_id, session_maker)
            if channel:
                rec = await recommend_posting_time(session_maker, channel.telegram_id)
                if rec.get("recommended_time"):
                    from datetime import timedelta

                    from app.core.time import utc_now

                    now = utc_now()
                    h, m = map(int, rec["recommended_time"].split(":"))
                    target = now.replace(hour=h, minute=m, second=0, microsecond=0)
                    if target <= now:
                        target += timedelta(days=1)
                    best_time_mins = int((target - now).total_seconds() / 60)
                    conf = rec.get("confidence", "")
                    best_time_label = f"🎯 Best: {rec['recommended_time']} ({conf})"
        except Exception:  # noqa: S110
            pass  # Non-critical, just skip the recommendation
        ...
        rows = []
        if best_time_label and best_time_mins > 0:
            rows.append(
                [
                    InlineKeyboardButton(
                        text=best_time_label,
                        callback_data=SchedulePreset(post_id=post_id, minutes=best_time_mins).pack(),
                    )
                ]
            )
        rows.extend(
        ...
        schedule_kb = InlineKeyboardMarkup(inline_keyboard=rows)
