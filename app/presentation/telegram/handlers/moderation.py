...
async def require_reply_target(message: types.Message, action: str) -> types.User | None:
    """Validate that the command is a reply to a user's message. Returns target user or None."""
    if not message.reply_to_message:
        await message.answer(f"Примените команду ответом на сообщение пользователя, которого нужно {action}. 🙏")
        await message.delete()
        return None
    if not message.reply_to_message.from_user:
        await message.answer("🚫 Это не пользователь или что-то пошло не так.")
        return None
...
async def mute_user(message: types.Message, bot: Bot) -> None:
    target_user = await require_reply_target(message, "замутить")
    if not target_user:
        return
    ...
    try:
        ...
        logger.error("mute_failed: error=%s user_id=%s chat_id=%s", err, target_user.id, message.chat.id)


@moderation_router.message(Command("unmute", prefix="!/"))
async def unmute_user(message: types.Message) -> None:
    target_user = await require_reply_target(message, "размутить")
    if not target_user:
        return
    ...
    try:
        ...
        await message.answer("Произошла ошибка. Попробуйте позже.")
        logger.error("unmute_failed: error=%s user_id=%s chat_id=%s", err, target_user.id, message.chat.id)


@moderation_router.message(Command("ban", prefix="!/"))
async def ban_user(message: types.Message, bot: Bot) -> None:
    target_user = await require_reply_target(message, "забанить")
    if not target_user:
        return
    ...
    try:
        ...
        logger.error("ban_failed: error=%s user_id=%s chat_id=%s", err, target_user.id, message.chat.id)
...
async def unban_user(message: types.Message, bot: Bot) -> None:
    target_user = await require_reply_target(message, "разбанить")
    if not target_user:
        return
    ...
    try:
        ...
        error_msg = await message.answer("Что-то пошло не так. Попробуйте позже.")
        logger.error("unban_failed: error=%s user_id=%s chat_id=%s", err, target_user.id, message.chat.id)
        other.sleep_and_delete(error_msg, 10)
...
async def full_ban(message: types.Message, message_repo: MessageRepository, db: AsyncSession) -> None:
    target_user = await require_reply_target(message, "добавить в черный список")
    if not target_user:
        return

    id_user = target_user.id
    reply_msg = message.reply_to_message
    assert reply_msg is not None  # guaranteed by require_reply_target
    chats_count = await message_repo.count_user_chats(id_user)
    messages_count = await message_repo.count_user_messages(id_user)
    spam_flag = await spam_service.detect_spam(db, reply_msg)
...
async def label_spam(message: types.Message, message_repo: MessageRepository, db: AsyncSession) -> None:
    target_user = await require_reply_target(message, "пометить как спам")
    if not target_user:
        return

    target = message.reply_to_message
    assert target is not None  # guaranteed by require_reply_target
    spammer_user_id = target_user.id
