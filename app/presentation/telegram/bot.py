...
async def on_shutdown(bot: Bot) -> None:
    ...
    try:
        # Stop analytics collector first (it uses Telethon + DB)
        analytics = container.get_analytics_collector()
        if analytics:
            await analytics.stop()

        # Stop channel orchestrator (it uses DB + LLM)
        ...
        # Cancel pending escalation timeout tasks
        if settings.moderation.enabled:
            from app.moderation.escalation import EscalationService

            await EscalationService.cancel_all_timeout_tasks()
...
async def main() -> None:
    ...
    set_usage_session_maker(session_maker)
    ...
    # Phase 3b: Start weekly report scheduler for active channels
    if settings.channel.enabled:
        try:
            from app.agent.channel.channel_repo import get_active_channels
            from app.agent.channel.reports import ReportScheduler

            active = await get_active_channels(session_maker)
            if active:
                report_scheduler = ReportScheduler(
                    bot=main_bot,
                    session_maker=session_maker,
                    channel_ids=[ch.telegram_id for ch in active],
                )
                report_scheduler.start()
                logger.info("report_scheduler_started", channels=len(active))
        except Exception:
            logger.exception("report_scheduler_init_failed")
    ...
    if assistant_enabled:
        ...
        if assistant_pair:
            ...
            # 2. commands_router (/stats, /sources, /settings) — explicit commands before AI agent
            # 3. assistant router (generic F.text catch-all)
            from app.assistant.bot import router as assistant_router
            from app.assistant.commands import commands_router
            from app.presentation.telegram.handlers.channel_review import channel_review_router

            assistant_dp.include_router(channel_review_router)
            assistant_dp.include_router(commands_router)
